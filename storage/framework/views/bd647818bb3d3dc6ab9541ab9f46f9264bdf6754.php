...
<?php $__env->startSection('pageName', 'Ruangan'); ?>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('#tbl1').DataTable({
                ...
                processing: true,
                serverSide: false,
                ajax: "<?php echo e(route('data.ruangan')); ?>",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        searchable: false
        ...
        function initAdd() {
            ...
            $('#form-ruang').attr('action', "<?php echo e(route('admin.ruangan.create')); ?>");
        ...
        async function initEdit(id) {
            ...
            $('#form-ruang').trigger('reset');
            $('#form-ruang').attr('action', "<?php echo e(route('admin.ruangan.update')); ?>");
            let res = await sendAjax("<?php echo e(route('data.ruangan.get')); ?>/" + id, {}, 'GET');
            if (res.status) {
                isiData(res.data);
                $('#modal_add').modal('show');
            } else {
        ...
        async function deleteRuang(idx) {
            let res = await sendAjax("<?php echo e(route('admin.ruangan.delete')); ?>", { id: idx });
            if (res.status) {
                $('#tbl1').DataTable().ajax.reload();
                alertSuccess(res.msg);
            } else {
...
<?php echo $__env->make('_layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\PROJECT\siadek_beta\resources\views/admin/ruangan.blade.php ENDPATH**/ ?>